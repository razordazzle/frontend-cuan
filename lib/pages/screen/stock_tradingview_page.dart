...
class StockTradingViewPage extends StatefulWidget {
  final String ticker;
  const StockTradingViewPage({super.key, required this.ticker});

  @override
  State<StockTradingViewPage> createState() => _StockTradingViewPageState();
}

class _StockTradingViewPageState extends State<StockTradingViewPage> {
  bool _isCandle = true;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<StockDetailProvider>().fetchTvCandles(widget.ticker);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<StockDetailProvider>(
        builder: (BuildContext context, StockDetailProvider p, _) {
          ...
          final Ohlc? liveBar = (candles.isNotEmpty && p.liveLast != null)
              ? Ohlc(
                  time: candles.last.time,
                  open: candles.last.open,
                  high: math.max(candles.last.high, p.liveLast!),
                  low: math.min(candles.last.low, p.liveLast!),
                  close: p.liveLast!,
                              ...
                              .map(
                                (String r) => Padding(
                                  ...
                                  child: ChoiceChip(
                                    ...
                                    onSelected: (_) => p.setTvResolution(widget.ticker,r),
                                  ),
                                ),
                              )
                              .toList(),
